...
public abstract class AbstractClient implements Runnable {
	...
	private static Log log = LogFactory.getLog(AbstractClient.class); ///log = Name des LogIners

	protected String userName; // Username (Login-Kennung) des Clients 
	///was ist der Unterschied zwischen userName und log ? 
	
	protected String threadName; /// anzahl thread = anzahl Client = anzahl Nachrichten von 1 Client? 

	protected int clientNumber;

	protected int messageLength;

	protected int numberOfMessagesToSend; ///soll unendlich sein? 
	...
	protected int numberOfRetries; ///Anzahl, wie viele Male wird die Nachricht abgesendet (im Störungsfall)?

	/**
	 * Denkzeit des Clients zwischen zwei Requests in ms
	 */
	protected int clientThinkTime; ///minimale Zeit zwischen Nachrichten von einem Cl.?
	...
	public AbstractClient(int serverPort, String remoteServerAddress, int clientNumber, int messageLength,
		...
		Thread.currentThread().setName("Client-" + String.valueOf(clientNumber + 1)); /// warum +1 ?
	...
	protected void watForLoggedInClients() throws InterruptedException {
		sharedData.getLoginSignal().countDown();
	...
	protected void waitForLoggingOutClients() throws InterruptedException {
		sharedData.getLogoutSignal().countDown(); /// ...()aus SharedClientStatistics + ...()countDownLatch
